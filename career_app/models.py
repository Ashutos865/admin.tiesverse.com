...
class FormApiKey(models.Model):
    """An API key that lets an external frontend (a different Tiesverse domain)
    submit to / read from a Form via /api/forms/v1/. Origin-locked, revocable,
    with optional expiry and single-use. Only the sha256 hash is stored; the
    full key is shown once at creation."""
    SCOPE_SUBMIT = 'submit'
    SCOPE_READ = 'read'
    SCOPE_CHOICES = [(SCOPE_SUBMIT, 'Submit only (POST)'), (SCOPE_READ, 'Read only (GET)')]

    form = models.ForeignKey(Form, on_delete=models.CASCADE, related_name='api_keys')
    label = models.CharField(max_length=120, blank=True)              # who / which site it's for
    scope = models.CharField(max_length=10, choices=SCOPE_CHOICES, default=SCOPE_SUBMIT)
    key_id = models.CharField(max_length=32, unique=True)            # public prefix, always visible
    key_hash = models.CharField(max_length=64)                       # sha256 hex of the full secret
    allowed_origins = models.JSONField(default=list, blank=True)     # ['https://x.com']; empty = blocked (must set one)
    expires_at = models.DateTimeField(null=True, blank=True)
    single_use = models.BooleanField(default=False)
    used_at = models.DateTimeField(null=True, blank=True)            # set when a single_use key is spent
    submissions_count = models.IntegerField(default=0)
    last_used_at = models.DateTimeField(null=True, blank=True)
    created_by_user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, db_constraint=False,
        related_name='form_api_keys',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    revoked_at = models.DateTimeField(null=True, blank=True)
    ...
    @classmethod
    def issue(cls, form, scope, **kwargs):
        """Create a key; returns (obj, full_key). full_key is shown once only."""
        import secrets
        prefix = 'tvf_sub' if scope == cls.SCOPE_SUBMIT else 'tvf_read'
        key_id = f'{prefix}_{secrets.token_hex(5)}'
        secret = secrets.token_urlsafe(32)
        full = f'{key_id}.{secret}'
        obj = cls.objects.create(form=form, scope=scope, key_id=key_id, key_hash=cls._hash(full), **kwargs)
        return obj, full
    ...
    def origin_allowed(self, origin):
        """True if the request Origin/Referer host matches an allowed origin.
        Empty allow-list = deny (a key must name at least one domain)."""
        if not origin:
            return False
